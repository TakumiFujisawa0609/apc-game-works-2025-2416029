...
void GameScene::Init() {
	...
	FinishSE =     LoadSoundMem("Data/SE/Finish.mp3");
}
...
void GameScene::SpawnBlock() {
    ...
    int type = nextBlock.front();
    nextBlock.erase(nextBlock.begin());
    nextBlock.push_back(rand() % 7);

    activeBlock.type = type;
    activeBlock.color = 1 + (rand() % 6);
    activeBlock.rotation = 0;

    for (int i = 0; i < 4; ++i) {
        activeBlock.shape[i][0] = minoShapes[type][i][0];
        activeBlock.shape[i][1] = minoShapes[type][i][1];
    }

    if (gravity == GRAVITY_DOWN) {
        activeBlock.px = BOARD_W / 2 - 2;
        activeBlock.py = 0;
    }
    else if (gravity == GRAVITY_UP) {
        activeBlock.px = BOARD_W / 2 - 2;
        activeBlock.py = BOARD_H - 4;
    }
    else if (gravity == GRAVITY_LEFT) {
        activeBlock.px = BOARD_W - 4;
        activeBlock.py = BOARD_H / 2 - 2;
    }
...
// ホールド機能
void GameScene::HoldBlock() {
    if (!canHold) return;

    //効果音を再生
    PlaySoundMem(HoldSE, DX_PLAYTYPE_BACK);

    static const int blockShapes[7][4][2] = {
        { {0,1}, {1,1}, {2,1}, {3,1} },
        { {1,0}, {2,0}, {1,1}, {2,1} },
        { {1,0}, {0,1}, {1,1}, {2,1} },
        { {1,0}, {2,0}, {0,1}, {1,1} },
    ...
    if (!hasHold) {
        ...
        for (int i = 0; i < 4; ++i) {
            activeBlock.shape[i][0] = blockShapes[activeBlock.type][i][0];
            activeBlock.shape[i][1] = blockShapes[activeBlock.type][i][1];
        }
    }
...
// ブロック回転
bool GameScene::RotateBlock(int dir) {
    int tmp[4][2];

    // 回転後の座標を計算
    for (int i = 0; i < 4; ++i) {
        int x = activeBlock.shape[i][0];
        int y = activeBlock.shape[i][1];
        if (dir == 1) {  // 右回転
            tmp[i][0] = 3 - y;
            tmp[i][1] = x;
        }
    ...
    // 各オフセットを試す
    for (int i = 0; i < 11; ++i) {
        test.px = activeBlock.px + (int)offsets[i].x;
        test.py = activeBlock.py + (int)offsets[i].y;
...
bool GameScene::IsGameOver() {
    return board.IsCollision(activeBlock, 0, 0);
}
...
void GameScene::HandleRotation() {
    ...
    if (input.IsKeyDown(KEY_INPUT_N) || input.IsKeyDown(XINPUT_BUTTON_A)) {
        bool success = RotateBlock(-1);
        if (success) {
            PlaySoundMem(BlockRotateSE, DX_PLAYTYPE_BACK);
        }
    }
    if (input.IsKeyDown(KEY_INPUT_M) || input.IsKeyDown(XINPUT_BUTTON_B)) {
        bool success = RotateBlock(1);
        if (success) {
            PlaySoundMem(BlockRotateSE, DX_PLAYTYPE_BACK);
        }
...
void GameScene::HandleFall() {
    ...
    if (fallCounter >= fallInterval) {
        ...
        // まず移動できるかチェック
        if (!board.IsCollision(activeBlock, ddx, ddy)) {
            // 移動できる場合
            activeBlock.px += ddx;
            activeBlock.py += ddy;
...
void GameScene::LandBlock() {
	...
	// パーティクル生成
    for (int i = 0; i < 4; ++i) {
        int x = activeBlock.px + activeBlock.shape[i][0];
        int y = activeBlock.py + activeBlock.shape[i][1];
...
void GameScene::HandleHardDrop() {
    ...
    if (hardDropInput) {
        ...
        ActiveBlock dropMino = activeBlock;
        int dropDistance = 0;
...
//ブロックの表示
void GameScene::DrawActiveBlock() {
    extern const int BOARD_OFFSET_X;
    extern const int BOARD_OFFSET_Y;
...
void GameScene::DrawUI() {
    ...
    if (hasHold) {
        ...
        for (int i = 0; i < 4; ++i) {
            int x = minoShapes[holdBlock.type][i][0];
            int y = minoShapes[holdBlock.type][i][1];
            DrawBox(
                holdX + x * BLOCK_SIZE,
                holdY + y * BLOCK_SIZE,
